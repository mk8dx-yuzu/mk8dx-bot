...
class Menu(discord.ui.View):
    ...
    async def btnffa(self, button: discord.ui.Button, interaction: Interaction):
        if interaction.user.mention not in self.bot.mogi["players"]:
            return await interaction.respond("Can't vote for that", ephemeral=True)

        if interaction.user.mention in self.bot.mogi["voters"]:
            return await interaction.respond("You voted already", ephemeral=True)

        self.bot.mogi["voters"].append(interaction.user.mention)
        self.bot.mogi["votes"][button.label.lower()] += 1
    ...
    async def btn2v2(self, button: discord.ui.Button, interaction: Interaction):
        if (
            not (self.count % 2 == 0 and self.count > 2)
        ) or interaction.user.mention not in self.bot.mogi["players"]:
            return await interaction.respond("Can't vote for that", ephemeral=True)
    ...
    async def btn3v3(self, button: discord.ui.Button, interaction: Interaction):
        if (
            not (self.count % 3 == 0 and self.count > 3)
        ) or interaction.user.mention not in self.bot.mogi["players"]:
            return await interaction.respond("Can't vote for that", ephemeral=True)
    ...
    async def btn4v4(self, button: discord.ui.Button, interaction: Interaction):
        if (
            not (self.count % 4 == 0 and self.count > 4)
        ) or interaction.user.mention not in self.bot.mogi["players"]:
            return await interaction.respond("Can't vote for that", ephemeral=True)
    ...
    async def btn6v6(self, button: discord.ui.Button, interaction: Interaction):
        if (
            not (self.count % 6 == 0 and self.count > 6)
        ) or interaction.user.mention not in self.bot.mogi["players"]:
            return await interaction.respond("Can't vote for that", ephemeral=True)
