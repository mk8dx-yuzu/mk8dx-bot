...
class funnies(commands.Cog):
    ...
    async def fun(self, ctx: ApplicationContext):
        ...
        with open("media/havingfun.png", "rb") as f:
            ...
            await ctx.respond(
                f"{ctx.interaction.user.mention} was caught having fun, they're now banned.",
                file=media,
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.channel.id == 1180622895316209664:
            if "dk summit" in message.content.lower() and random.random() < 0.4:
                return await message.channel.send(
                    random.choice(
                        ["DDDDKKKK SUMIIIIT", "dk summit mentioned", "mitsiku summit"]
                    )
                )

            if (
                " election" in message.content.lower()
                or message.content.lower().startswith("election")
            ) and random.random() < 0.5:
                return await message.channel.send(":flag_us:")

            if (
                self.bot.user.mention in message.content.lower()
            ...
            if "my dream" in message.content.lower():
                if 0.1 < random.random():
                    return await message.channel.send(
                        "eyo bro, sorry that im writing u directly, U seem to have the File to Zelda totk. i want to play it on my pc but im too dumb to get it running and i dont seem to have the right game files. can u help me out? maybe by a link to the gamefile and a little guide to set that shit up. would be my dream bro"
                    )
                return await message.channel.send(
                    "Hello, recently I've downloaded a rom of tears of the kingdom. But when I launch the game with yuzu. He says me this : “The titlekey for this rights ID could not be found” I've tried to get new keys but it's the same results. Can you help me pls ? (Sorry for my English, I'm French)"
                )

            if (
                "where" in message.content.lower()
                and "password" in message.content.lower()
            ):
                return await message.channel.send(
                    "plz password mario kart 8 Eu main ? :3 Use Google Translate. I have a hard time understanding English, there is a server that asks me for a password and it goes to this discord, I thought I would find it here. I am from Chile (Spanish)"
                )
        ...
        if (
            self.bot.mogi["status"]
            and len(self.bot.mogi["players"]) == 11
            and "9/11" in message.content.lower()
        ):
